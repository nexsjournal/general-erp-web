...
1. **Dashboard 参考图（两轮）**：
   - 初版 3 张（Social Orbit / Apex / FinanceFlow 风格）：浅色底 + 白卡片 + 细边框、蓝色主色、KPI 卡（大数字 + 涨跌徽章）、线性图表、圆角图标磁贴、分组侧边导航（MAIN/OTHERS）、顶栏（搜索 ⌘K + 通知 + 语言 + 头像）；
   - 2026-08 补充 3 张（Boltshift / Symptira / Elementor 风格，Behance「AI-Powered Healthcare OS」同类）：**疏朗布局**——更浅冷灰页面底、16-20px 大圆角卡片、20px+ 卡片间距、pill 化按钮/徽标、KPI 大数字（28-32px）+ 圆形图标芯片、宽松表格行（48px+）。当前主题按此轮执行。
...
- **疏朗优先（2026-08 起）**：反馈「默认太挤」后由紧凑模式调整为疏朗模式——列表行高常规 48px、卡片 16px 圆角 + 24px 内边距、栅格 gap 20px、KPI 数字 28-32px；表格正文仍保持 13px 保证信息承载力。超高密度场景可在组件级显式回退紧凑参数，不全局收紧。
