...
def sync_user_login_settings():
    ...
    #     （frappe 默认 170 小时 ≈ 7 天，客户反馈"过段时间不提示账号密码"的根因）
...
def sync_approval_wizard():
    ...
    for state_name in ("审批1", "审批2", "审批3"):
        if not frappe.db.exists("Workflow State", state_name):
            frappe.get_doc({"doctype": "Workflow State", "state": state_name}).insert(ignore_permissions=True)
    for action_name in ("审批通过", "免批通过"):
        if not frappe.db.exists("Workflow Action Master", action_name):
            frappe.get_doc({"doctype": "Workflow Action Master", "name": action_name}).insert(ignore_permissions=True)
    # ② 催办日志 DocType（JSON 随 app migrate 自动建；此处兜底新站点 install 场景）
    if not frappe.db.exists("DocType", "Approval Reminder Log"):
        import os
        import json as _json
        path = os.path.join(os.path.dirname(__file__), "doctype", "approval_reminder_log", "approval_reminder_log.json")
