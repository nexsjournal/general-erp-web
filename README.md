...
- **营销**：营销活动、邮件群发（模板变量 + 发送统计）、邮件模板、网站留言 Web Form（`/website-lead` + 官网嵌入代码页）
- **采购与库存**：供应商、采购订单、采购入库、来料验货单、物料（HS 编码）、入库/库存余额/出库
- **OA**：待办（ToDo）、公告、文件管理、工作检查（自检 + ToDo 提醒）、工作日历（节假日/跟进/出运）
- **邮件**：邮件中心（待处理/收件箱/草稿箱）、IMAP 邮箱账号接入、发送跟踪（打开像素 + 点击跳转）
- **基础平台**：港口、贸易术语、HS 编码、系统参数、币种汇率（今日汇率数字卡）、服务商、邮箱账号、统计设置（公海天数 + 报表订阅定时推送 + 报表可见角色）、区域设置（语言/时区/日期）
- **报表（14 个）**：外销/出运/订单利润/产品/采购/收款/付款/费用/客户/线索/商机/邮件统计、库存预警、员工业绩排行、员工工作情况表
- **工作台与演示**：外贸工作台（12 快捷方式 + 数字卡 + 图表）、业务流程页（按功能清单串联全部单据，可点击跳转）、侧边栏 37 项
