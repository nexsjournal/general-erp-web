...
## 最近更新（v1.1，2026-08-30）

完整清单见 [CHANGELOG.md](CHANGELOG.md)。本轮交付：

- **金蝶式首页**：登录后直达「首页」（今日概览数字卡 + 12 业务模块入口），/index 短路径直达，品牌统一「太康生物ERP」
- **会话超时**：闲置超 8 小时强制重新登录（原 frappe 默认 170 小时≈7 天）；System Settings.session_expiry=8:00，site_setup 幂等固化防 migrate 回滚
- **两级导航**：左侧功能域（销售/采购/库存/财务/客户/产品/邮件/外贸/生产/组织/报表/设置）+ 右侧子功能磁贴
- **岗位角色体系**：7 个岗位（销售/外贸专员/采购/库存/财务/总经理/系统管理员），建号选岗位自动带整套权限；新增「流程设计」角色
- **用户隐私权限**（2026-08-30 拍板）：普通用户用户列表只看到自己，改自己资料/密码不受影响；读他人 403
- **帮助中心**：侧栏「帮助中心」入口，快速上手 3 步 + 13 模块分步教程 + 常用操作速查 + FAQ
- **轻量流程配置**：每个模块页顶部显示可后台编辑的流程步骤条（Module Flow），改完即时生效
- **数据与备份**：系统设置内备份列表/一键备份/下载，每日 03:00 自动备份（保留 14 天，含恢复演练）
- **金额口径**：CNY 显示「数字 + 元」（如 492,000.00 元），汇率卡 2 位小数，图表坐标轴千分位
- **质量门禁**：44 项全模块回归 scripts/regression.sh 一键跑；graph 两轮全角度测试（83 页 + 四路并行 + 独立验证）0 P0/P1
## 文档

- [首次访问指南：登录页与初始配置](docs/first-run-guide.md)
 - [变更历史 CHANGELOG.md](CHANGELOG.md)（每轮交付的功能/修复/测试结论）
 - [graph 全角度测试报告（2026-08-30）](docs/reports/graph测试报告-2026-08-30.md)
...
- **工作台与演示**：ERP工作台（15 模块卡片 + 12 快捷方式 + 5 数字卡 + 图表）、商机批复（待批复/已批复/待回复三态 + 留痕 + 列表筛选视图）、业务流程页（按功能清单串联全部单据，可点击跳转）、侧边栏 37 项、用户参数入口（User 偏好设置）
...
- 普通用户登录直达「首页」；用户列表只看到自己，改自己的资料/密码不受影响，看他人资料 403。
- 需要管人/管流程的（老板、主管、IT）加「流程设计」或对应岗位角色。
- 侧栏「ERP工作台」仅系统管理员可见（维护入口）；业务入口走「首页」。
